...
use Crud\Forms\Order;
use Crud\Forms\Filter;
use Crud\Config;
 ...
 *               \Crud\Model\InterfaceModel or (easier with Db_Table)
 *               extends \Crud\Model\Abstract
 * _getForm() must return a form that must extend Crud\Forms\AbstractForm
 *
 */
/**
 * Class Name
    ...
    extends \Zend_Controller_Action
{
        ...
        array $invokeArgs = array())
    {
        ...
        if (!($this->CrudModel() instanceof \Crud\Model\InterfaceModel)) {
            throw new \Zend_Exception(
                'object not an instance of \Crud\Model\InterfaceModel'
            );
        }
        $this->_setupPK();
        // generate form
        $this->_crudForm = $this->_getCrudForm();
        if (!($this->CrudForm() instanceof \Crud\Forms\AbstractForm)) {
            throw new \Zend_Exception(
                'object not an instance of Crud\Forms\AbstractForm'
            );
        }
     ...
     *
     * @return \Crud\Model\InterfaceModel CRUD model
     */
     ...
     *
     * @return Crud\Forms\AbstractForm CRUD form
     */
    ...
    */
    ...
    /**
    * Method called after edit delete action is performed
    */
    ...
    protected function _processOrderForm()
    {
       ...
       if ( Order\Helper::isFormPosted($this->getRequest())) {
         $this->getHelper('redirector')->gotoUrl(
             Order\Helper::url($this->view, $this->getRequest())
         );
       }
       $orderQuery =
           Order\Helper::getOrderQuery($this->getRequest());
       $this->view->orderForm =
    ...
    {
        //filters
       if (Filter\Helper::isFormPosted($this->getRequest())) {
            $filtersWhereArray = Filter\Helper::arrayToWhereArray(
       ...
       $filterArray = Filter\Helper::stringToArray(
    ...
    public function indexAction()
    {
       ...
       $this->view->addHelperPath(
           realpath(dirname(__FILE__).'/../Forms/Helpers/ListHelpers/'), //to move
           'Crud\\Forms\\Helpers\\ListHelpers\\'
       );
       
       //set adapter, paginator,
       if ($this->CrudModel() instanceof \Crud\Model\DbTable\AbstractDbTable) {
           $adapter = $this->CrudModel()->fetchPaginatorAdapter(
               $filtersWhereArray, $orderQuery, array()
           );
       }
       /*if ($this->CrudModel() instanceof Crud\Model\Rest_Abstract) {
           $adapter = $this->CrudModel()->fetchPaginatorAdapter(
       ...
       $this->view->data = \Crud\View\Helpers\Paginator::paginatorToArray(
           $this->view->paginator
       );
    ...
    public function addAction()
    {
        ...
        try {
            ...
            if (strpos($e->getMessage(), 'add.phtml')!==false) {
                ...
                echo '<a href="'
                     .$this->view->url(
                         array(Config::PK_NAME=>null, 'action'=>'index')
                     )
    ...
    public static function getNeededValuesForPK(
            \Crud\Model\InterfaceModel $model, array $postData
    )
    {
        $pk = $model->getPKName();
        if (is_array($pk)) {
    ...
    public function deleteAction()
    {
        if ($this->getRequest()->isPost()) {
            ...
            $pkRawValue = $this->_getParam(Config::PK_NAME);
            ...
            try {
                ...
                if (strpos($e->getMessage(), 'delete.phtml')!==false) {
                    ...
                    <div>
                      <input type="hidden" name="' . Config::PK_NAME
