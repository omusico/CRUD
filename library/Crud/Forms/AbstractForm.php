    ...
    implements InterfaceForm
{
     ...
     */
    //const Application*_DATE_FORMAT = "Y-m-d H:i:s";
    //const Application*_TIMESTAMP_FORMAT = "Y-m-d H:i:s";
    ...
    public function __construct($config = array())
    {
        ...
        if (!($this->_model instanceof \Crud\Model\InterfaceModel)) {
            throw new \Zend_Exception(
                'model not an instance of \Crud\Model\InterfaceModel'
                ...
                // fields
                    ...
                    if (
                        in_array(
                            ...
                            array('text', 'tinytext', 'mediumtext', 'longtext')
                        )
                    ) {
                        $element = new \Zend_Form_Element_Textarea($field);
                        $element->setAttrib('rows', 3)->setAttrib('cols', 40);
                    } else if (
                        preg_match('/^enum/i', $this->_currentRow['DATA_TYPE'])
                    ) {
                        // extract  enum('waiting','running','done') -> array 
                        preg_match_all('/([^\',\(\)]+)/', $this->_currentRow['DATA_TYPE'], $m);
                        if (!preg_match('/^enum/i', $m[0][0])) {
                            throw new \Zend_Exception("cannot parse column type" . print_r($this->_currentRow, 1));
                        }
    ...
    protected function addAjaxJs($options)
    {
        if (isset($this->_fixedValues[$options['id']])) {
           return '';
            ...
            return  Crud\Forms\Helpers\AutoComplete::getJsAfterForm(
                $options
            );
        }
    }
